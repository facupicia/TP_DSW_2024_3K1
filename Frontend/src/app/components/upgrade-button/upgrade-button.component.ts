...
@Component({
    ...
    template: `
    <!-- Upgrade Button -->
    <button *ngIf="!isPro" (click)="openModal()" [class]="buttonClass">
      <ng-content></ng-content>
      <span *ngIf="!hasContent">🚀 Mejorar a PRO</span>
    </button>

    <!-- Already PRO indicator -->
    <span *ngIf="isPro && showProBadge"
      class="px-3 py-1 bg-gradient-to-r from-purple-500 to-indigo-600 text-white rounded-full text-xs font-bold">
      ⭐ PRO
    </span>

    <!-- Billing Modal -->
    <div *ngIf="showBillingModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div class="relative bg-white rounded-3xl w-full max-w-md p-8 shadow-2xl animate-scale-in">
        ...
        <div class="space-y-4">
          ...
          <button *ngIf="selectedPlan?.yearlyPrice" (click)="checkout('yearly')" [disabled]="loading"
        ...
        <div class="mt-6 pt-4 border-t border-gray-100 flex justify-between items-center">
          ...
          <div *ngIf="loading" class="flex items-center gap-2 text-sm font-medium text-indigo-600">
          ...
          <span *ngIf="!loading" class="text-[10px] text-gray-400">Pagos seguros con MercadoPago</span>
        </div>
      </div>
    </div>
  `,
